...
ggplotly(plot_totals_jitter_sampled +
  ...
  tooltip="text") %>% layout(margin=list(b=80, l=80, r=80))
```
...
plot_totals_density <- totals %>%
  ...
  scale_x_log10() +
  labs(x="&nbsp;<br>Seconds to reach Stage", y="Density<br>&nbsp;")

ggplotly(plot_totals_density, tooltip="text") %>% layout(margin=list(b=80, l=80, r=80))
```
...
#### Time To First Contentful Paint — Normalized Contributors by Quantile
...
```{r echo=FALSE, warning=FALSE}
ggplotly(ci_plots$contributors_by_quantile, tooltip = "text") %>% layout(margin=list(b=80, l=80, r=80))
```

## Time To Interactive | Normalized by quantiles
```{r echo=FALSE, warning=FALSE}
ggplotly(ci_plots$contributors_by_quantile_normalized, tooltip="text") %>% layout(margin=list(b=80, l=80, r=80))
```
...
ggplotly(plot_per_second, tooltip="text")  %>% layout(margin=list(b=80, l=80, r=80))
```
...
```{r echo=FALSE, warning=FALSE}
ggplotly(plot_per_second_worst100, tooltip="text")  %>% layout(margin=list(b=80, l=80, r=80))
```

#### Mean Contributors over time (Best 100 sites)

```{r echo=FALSE, warning=FALSE}
ggplotly(plot_per_second_best100, tooltip="text")  %>% layout(margin=list(b=80, l=80, r=80))
```
